...
class DataLoader:
    ...
    def _find_posting_date_column(df):
        """Busca una columna de posting date o fecha de libro mayor en un DataFrame."""
        for col in df.columns:
            col_lower = str(col).lower()
            if "posting" in col_lower:
                return col
            if any(p in col_lower for p in ["libro mayor", "fecha lm", "fechalm", "gl date", "g/l date", "fecha gl", "fecha g/l"]):
                return col
        return None
