...
class ControlApp:
    ...
    def _create_widgets(self):
        ...
        self.btn_update_report = ttk.Button(exec_frame, text="Actualizar Reporte", command=self.update_report, state=tk.DISABLED)
    ...
    def update_report(self):
        ...
        if not self.last_out_path:
            messagebox.showwarning("Advertencia", "No hay reporte previo para actualizar.")
            return
        
        try:
            # Re-ejecutar el proceso, pero indicando que es una actualización
            self.run_process_thread(is_update=True)
        except Exception as e:
            logger.error(f"Error al intentar actualizar el reporte: {e}", exc_info=True)
            messagebox.showerror("Error", f"Error al actualizar el reporte:\n{e}")
