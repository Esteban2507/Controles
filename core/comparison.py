...
def classify_status(row, tolerance, report_type="CDP"):
    ...
    # Comparar tipo de cambio si existe en ambos lados (no se aplica para E1)
    if report_type != "E1" and not _exchange_rates_match(row.get(f"tipo_cambio{suffix_reporte}"), row.get("tipo_cambio_ARCA")):
