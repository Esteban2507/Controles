...
def process_data(config, arca_path, cdp_path, e1_path="", fecha_filtro=None):
    ...
    # Identificar valores duplicados en la columna 'duplicados' de los reportes de ERP (CDP y/o E1)
    erp_dups = set()
    if "duplicados" in df_reporte_all.columns:
        val_dups = df_reporte_all["duplicados"].dropna().astype(str).str.strip()
        val_dups = val_dups[(val_dups != "") & (val_dups.str.lower() != "nan") & (val_dups.str.lower() != "none") & (val_dups.str.lower() != "<na>")]
        val_counts = val_dups.value_counts()
        erp_dups = set(val_counts[val_counts >= 2].index)
    ...
    if not df_cdp.empty:
        ...
        # Aplicar control de duplicados de ERP (columna duplicados)
        if erp_dups:
            mask_dup = df_merge_cdp["Concatenado"].astype(str).apply(
                lambda c: any(str(k) in erp_dups for k in cdp_keys_by_concat.get(c, []))
            )
            df_merge_cdp.loc[mask_dup, "Status"] = "Posible duplicado"
    ...
    if not df_e1.empty:
        ...
        # Aplicar control de duplicados de ERP (columna duplicados)
        if erp_dups:
            mask_dup = df_merge_e1["Concatenado"].astype(str).apply(
                lambda c: any(str(k) in erp_dups for k in e1_keys_by_concat.get(c, []))
            )
            df_merge_e1.loc[mask_dup, "Status"] = "Posible duplicado"
