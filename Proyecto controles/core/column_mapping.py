...
def robust_rename(df: pd.DataFrame, mapping: dict, sistema: str, required_cols: list) -> pd.DataFrame:
    ...
    if "vendor" in required_cols and "vendor" not in df.columns:
        ...
        for col in df.columns:
            c = canonicalize(col)
            # Debe contener receptor, proveedor, vendor, o denominación
            if any(p in c for p in ["receptor", "proveedor", "vendor", "denominacion"]):
                # Pero no debe ser una columna de metadatos de documentos como tipo de documento, nro de documento, cuit o códigos
                if not any(p in c for p in ["tipo", "nro", "doc", "cuit", "codigo", "cod"]):
                    candidates.append(col)
